...
function TableRow({ className, ...props }: React.ComponentProps<'tr'>) {
  return (
    <tr
      data-slot="table-row"
      className={cn('border-b border-[#374151] transition-colors hover:bg-[#1F2A37] data-[state=selected]:bg-neutral-800', className)}
      {...props}
    />
  );
}
...
function TableHead({ className, children, textAlign = 'left', ...props }: TableHeadProps) {
  ...
  return (
    <th className={cn('h-12 p-4 align-middle whitespace-nowrap [&:has([role=checkbox])]:pr-0', alignmentClass, className)} {...props}>
